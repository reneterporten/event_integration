...
cfgfreq = [];
cfgfreq.method = 'mtmconvol';
cfgfreq.foi    = 30:2.5:90;
cfgfreq.t_ftimwin = ones(1,numel(cfgfreq.foi)).*0.4;
cfgfreq.taper  = 'dpss';
cfgfreq.tapsmofrq = ones(1,numel(cfgfreq.foi)).*10;
cfgfreq.toi    = (-150:15:450)./300;
cfgfreq.pad    = 4;
...
for k = 1:numel(subjlist)
  ...
  qsubfeval('rt_sensorlevelanalysis', subjlist{k}, 'type', 'tfr', 'saveflag', savedir, 'savename', 'tfr_high', 'cfgfreq', cfgfreq, 'memreq', 12*1024^3, 'timreq', 60*60, 'batchid', subjlist{k});
