function c = qsub_trc(x,y,n,subj,chan)
...
corrchan = squeeze(nanmean(nanmean(c,3),2));

savedir = '/project/3012026.13/jansch';
mkdir(fullfile(savedir, subj))
save(fullfile(savedir, subj, sprintf('%s_corr_mcca_chan_%d', subj, chan)), 'corrchan');